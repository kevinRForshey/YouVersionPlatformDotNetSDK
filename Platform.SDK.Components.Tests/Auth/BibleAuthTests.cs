...
public sealed class BibleAuthTests : TestContext
{
    private readonly Mock<IAuthSessionService> _authSessionService = new();

    private IRenderedComponent<BibleAuth> RenderAuth(Action<ComponentParameterCollectionBuilder<BibleAuth>>? configure = null)
    {
        Services.AddSingleton(_authSessionService.Object);
        return configure is null ? RenderComponent<BibleAuth>() : RenderComponent<BibleAuth>(configure);
    }
