...
A minimal console sample showing how to consume the [YouVersion Platform SDK for .NET](../README.md)
...
- .NET 10 SDK
- A YouVersion app key from the [YouVersion developer portal](https://developers.youversion.com)
...
| Fetch a passage | `IPassageClient.GetPassageAsync`, using `Reference.FromString` from `YouVersion.UsfmReferences` |

DI wiring is a single call — `AddYouVersionApiClients(builder.Configuration)` — since this sample
doesn't need OAuth or caching. See [`Platform.API/README.md`](../Platform.API/README.md) for the
full set of `AddYouVersion*` extensions (OAuth, caching, resilience) available when you need them.
