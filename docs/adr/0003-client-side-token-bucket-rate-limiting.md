...
The YouVersion Platform API enforces its own server-side rate limits and will return `429` once a
...
client's `HttpClient` pipeline (`AddYouVersionApiClients`). Requests that can't acquire a token within
...
hardcoded, since they need to match whatever tier of YouVersion's API a given consumer is provisioned
for.

## Consequences

- Consumers get predictable local throttling behavior without needing to implement their own limiter or
  read YouVersion's rate-limit documentation before their first production incident.
- The local limit is independent of the server's actual limit and must be configured to a value the
  consumer believes matches it (`YouVersionApiOptions.OutboundRequestsPerSecond`/`OutboundBurstSize`) —
